...
func TestProviderName(t *testing.T) {
	p := &Provider{}
	if got := p.Name(); got != "cert_eu" {
		t.Errorf("Name() = %q, want %q", got, "cert_eu")
	}
}
...
func TestBuildControl(t *testing.T) {
	p := &Provider{}
	ctrl := p.buildControl(certEUAdvisory{
		ID: "CERT-EU-99", Title: "Test", Family: "Test Family",
		Description: "Desc", Level: "critical", Type: "Threat Advisory", TLP: "AMBER",
		IOCs: []string{"CVE-2024-0001"},
	})
	if ctrl.ControlID != "CERT-EU-99" {
		t.Errorf("expected ControlID CERT-EU-99, got %s", ctrl.ControlID)
	}
	...
	if len(ctrl.RelatedCVEs) != 1 {
		t.Errorf("expected 1 RelatedCVE, got %d", len(ctrl.RelatedCVEs))
	}
	if ctrl.RelatedCVEs[0] != "CVE-2024-0001" {
		t.Errorf("expected CVE-2024-0001, got %s", ctrl.RelatedCVEs[0])
	}
	if ctrl.References[0].Section != "Threat Advisory" {
		t.Errorf("unexpected reference section: %s", ctrl.References[0].Section)
	}
}

func TestBuildControlNoIOCs(t *testing.T) {
	p := &Provider{}
	ctrl := p.buildControl(certEUAdvisory{
		ID: "CERT-EU-00", Title: "Test", Family: "F", Description: "D",
	})
	if len(ctrl.RelatedCVEs) != 0 {
		t.Errorf("expected 0 RelatedCVEs, got %d", len(ctrl.RelatedCVEs))
	}
}

func TestParseSuccess(t *testing.T) {
	catalog := certEUCatalog{
		Advisories: []certEUAdvisory{
			{ID: "CERT-EU-01", Title: "Test", Family: "Family", Description: "Desc", Level: "high", Type: "Advisory", IOCs: []string{"CVE-2024-0001"}},
		},
	}
...
func TestDownloadSuccess(t *testing.T) {
	catalog := certEUCatalog{Advisories: []certEUAdvisory{{ID: "01", Title: "T"}}}
	data, _ := json.Marshal(catalog)
	...
	f, err := os.CreateTemp("", "cert_download_*.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDownloadNon200(t *testing.T) {
	...
	p := &Provider{logger: testLogger()}
	f, err := os.CreateTemp("", "cert_download_err_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
...
func TestDownloadIOCopyError(t *testing.T) {
	...
	err = p.download(context.Background(), server.URL, dest)
	if err == nil {
