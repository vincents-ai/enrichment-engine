...
func TestProviderName(t *testing.T) {
	p := &Provider{}
	if got := p.Name(); got != "enisa_cra_mapping" {
		t.Errorf("Name() = %q, want %q", got, "enisa_cra_mapping")
	}
}
...
func TestParse(t *testing.T) {
	catalog := mappingCatalog{
		Mappings: []craMapping{
			{ID: "MAP-001", CRARequirement: "Annex I-1", HarmonisedStd: "EN 18031-1", StdSection: "5.1", Title: "Secure by Design", Description: "Desc", Confidence: "high", Level: "critical", Annex: "I"},
			{ID: "MAP-002", CRARequirement: "Annex I-2", HarmonisedStd: "EN 18031-1", StdSection: "6.1", Title: "Secure Default", Description: "Desc 2", Confidence: "medium", Annex: "I"},
		},
	}
	...
	if controls[0].ControlID != "MAP-001" {
		t.Errorf("expected ControlID MAP-001, got %s", controls[0].ControlID)
	}
	if controls[0].Level != "critical" {
		t.Errorf("expected Level critical, got %s", controls[0].Level)
	}
	if controls[1].Level != "standard" {
		t.Errorf("expected Level standard for medium confidence, got %s", controls[1].Level)
	}
	if controls[0].ImplementationGuidance == "" {
		t.Error("expected non-empty ImplementationGuidance")
	}
	if !strings.Contains(controls[0].Family, "EN 18031-1") {
		t.Errorf("expected Family to contain standard name, got %s", controls[0].Family)
	}
}
...
func TestBuildControl(t *testing.T) {
	...
	tests := []struct {
		...
		{"with level", craMapping{Level: "high"}, "high"},
		{"high confidence", craMapping{Confidence: "high"}, "high"},
		{"medium confidence", craMapping{Confidence: "medium"}, "standard"},
		{"unknown confidence", craMapping{Confidence: "low"}, "standard"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctrl := p.buildControl(tt.m)
			if ctrl.Level != tt.wantLvl {
				t.Errorf("expected Level %s, got %s", tt.wantLvl, ctrl.Level)
			}
			if ctrl.Framework != FrameworkID {
				t.Errorf("expected Framework %s, got %s", FrameworkID, ctrl.Framework)
			}
			if len(ctrl.References) != 2 {
				t.Errorf("expected 2 references, got %d", len(ctrl.References))
			}
		})
	}
}
...
func TestDownloadSuccess(t *testing.T) {
	body := `{"mappings":[{"id":"MAP-001","title":"Test","description":"desc"}]}`
	...
	f, err := os.CreateTemp("", "enisa_cra_download_*.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDownloadNon200(t *testing.T) {
	...
	p := &Provider{logger: testLogger()}
	f, err := os.CreateTemp("", "enisa_cra_download_err_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
...
func TestRunDownloadSuccess(t *testing.T) {
	catalog := mappingCatalog{
		Mappings: []craMapping{
			{ID: "MAP-001", CRARequirement: "Annex I-1", HarmonisedStd: "EN 18031-1", StdSection: "5.1", Title: "Secure by Design", Description: "Desc", Level: "critical", Annex: "I"},
			{ID: "MAP-002", CRARequirement: "Annex I-2", HarmonisedStd: "EN 18031-1", StdSection: "6.1", Title: "Secure Default", Description: "Desc 2", Annex: "I"},
		},
	}
...
func TestRunParseErrorFallback(t *testing.T) {
	...
	embeddedCount := len(p.generateEmbeddedMappings())
