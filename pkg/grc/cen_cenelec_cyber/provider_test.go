...
func TestProviderName(t *testing.T) {
	p := &Provider{}
	if got := p.Name(); got != "cen_cenelec_cyber" {
		t.Errorf("Name() = %q, want %q", got, "cen_cenelec_cyber")
	}
}
...
func TestBuildControl(t *testing.T) {
	p := &Provider{}
	ctrl := p.buildControl(cyberStd{
		ID: "CC-CYB-99", Title: "Test", Family: "Test Family",
		Description: "Desc", Level: "high", StdRef: "EN ISO/IEC 27001, Clause 99",
	})
	if ctrl.ControlID != "CC-CYB-99" {
		t.Errorf("expected ControlID CC-CYB-99, got %s", ctrl.ControlID)
	}
...
func TestParseSuccess(t *testing.T) {
	catalog := cyberStdCatalog{
		Standards: []cyberStd{
			{ID: "CC-CYB-01", Title: "Test 1", Family: "Family 1", Description: "Desc 1", Level: "critical", StdRef: "Ref 1"},
		},
	}
...
func TestDownloadSuccess(t *testing.T) {
	catalog := cyberStdCatalog{Standards: []cyberStd{{ID: "01", Title: "T"}}}
	data, _ := json.Marshal(catalog)
	...
	f, err := os.CreateTemp("", "cyber_download_*.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDownloadNon200(t *testing.T) {
	...
	p := &Provider{logger: testLogger()}
	f, err := os.CreateTemp("", "cyber_download_err_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
