...
// New creates a new ISO 27001 provider.
...
// Run fetches the ISO 27001 catalog, parses controls, and writes them to storage.
func (p *Provider) Run(ctx context.Context) (int, error) {
	...
	p.logger.Info("parsed ISO 27001 controls", "count", len(controls))
	...
	p.logger.Info("wrote ISO 27001 controls to storage", "count", count)
	return count, nil
}
...
type iso27001Catalog struct {
	Controls []iso27001Control `json:"controls"`
	Groups   []iso27001Group   `json:"groups,omitempty"`
}

type iso27001Group struct {
	ID       string            `json:"id"`
...
func (p *Provider) parse(path string) ([]grc.Control, error) {
	...
	var catalog iso27001Catalog
	if err := json.NewDecoder(f).Decode(&catalog); err != nil {
		return nil, fmt.Errorf("decode ISO 27001 catalog: %w", err)
	}
	...
	for _, group := range catalog.Groups {
		for _, ctrl := range group.Controls {
			controls = append(controls, p.toControl(ctrl, group.Title))
		}
	}

	if len(controls) == 0 {
		for _, ctrl := range catalog.Controls {
			controls = append(controls, p.toControl(ctrl, ctrl.Theme))
		}
	}
...
func (p *Provider) toControl(ctrl iso27001Control, family string) grc.Control {
	description := ctrl.Description
	if description == "" && ctrl.Purpose != "" {
		description = ctrl.Purpose
	}
	if ctrl.Guidance != "" {
...
func (p *Provider) writeEmbeddedControls(ctx context.Context) (int, error) {
	p.logger.Info("using embedded ISO 27001:2022 controls")
...
var isoCWEMap = map[string][]string{
	...
	"A.8.25": {"CWE-94", "CWE-78", "CWE-95", "CWE-1059", "CWE-119", "CWE-502"},
	"A.8.26": {"CWE-1059", "CWE-693"},
	"A.8.27": {"CWE-1059", "CWE-693"},
	"A.8.28": {"CWE-94", "CWE-78", "CWE-95", "CWE-119", "CWE-1336", "CWE-502"},
...
func embeddedControls() []grc.Control {
	...
	organizational := []struct{ id, title, desc string }{
		...
		{"A.5.9", "Inventory of information and other associated assets", "An inventory of information and other associated assets shall be developed and maintained."},
		...
		{"A.5.17", "Authentication information", "Allocation and management of authentication information shall be controlled by a management process, including advising personnel and other interested parties on appropriate handling of authentication information."},
		...
		{"A.5.34", "Privacy and protection of PII", "The organization shall identify and meet the requirements regarding the preservation of privacy and protection of PII according to applicable laws and regulations."},
		{"A.5.35", "Independent review of information security", "The organization's approach to managing information security and its implementation including people, processes and technologies shall be reviewed independently at planned intervals."},
	...
	people := []struct{ id, title, desc string }{
		{"A.6.1", "Screening", "Background verification checks on all candidates to become personnel shall be carried out prior to joining the organization and on an ongoing basis taking into account applicable laws, regulations and ethics and be proportional to the business requirements, the classification of the information to be accessed and the perceived risks."},
	...
	physical := []struct{ id, title, desc string }{
		{"A.7.1", "Physical security perimeters", "Security perimeters shall be defined and used to protect areas that contain either sensitive or critical information and other associated assets."},
		...
		{"A.7.13", "Equipment maintenance", "Equipment shall be maintained correctly and regularly to ensure availability, integrity and confidentiality of information."},
	...
	technological := []struct{ id, title, desc string }{
		...
		{"A.8.5", "Secure authentication", "Authenticated information systems shall be protected from the effects of brute force attacks by implementing appropriate mitigations."},
		...
		{"A.8.20", "Addresses of networks and services", "IP addresses and logical names of networked services shall be managed."},
		...
		{"A.8.33", "Test information", "Test information shall be appropriately protected and managed."},
