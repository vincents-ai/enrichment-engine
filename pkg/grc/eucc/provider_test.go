...
func TestProviderName(t *testing.T) {
	p := &Provider{}
	if got := p.Name(); got != "eucc" {
		t.Errorf("Name() = %q, want %q", got, "eucc")
	}
}
...
func TestBuildControl(t *testing.T) {
	p := &Provider{}
	ctrl := p.buildControl(euccControl{
		ID: "EUCC-99", Title: "Test", Family: "Test Family",
		Description: "Desc", Level: "high", Category: "TestCategory",
	})
	if ctrl.ControlID != "EUCC-99" {
		t.Errorf("expected ControlID EUCC-99, got %s", ctrl.ControlID)
	}
...
func TestParseSuccess(t *testing.T) {
	catalog := euccCatalog{
		Controls: []euccControl{
			{ID: "EUCC-01", Title: "Test", Family: "Family", Description: "Desc", Level: "critical", Category: "Cat"},
		},
	}
...
func TestDownloadSuccess(t *testing.T) {
	catalog := euccCatalog{Controls: []euccControl{{ID: "01", Title: "T"}}}
	data, _ := json.Marshal(catalog)
	...
	f, err := os.CreateTemp("", "eucc_download_*.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDownloadNon200(t *testing.T) {
	...
	p := &Provider{logger: testLogger()}
	f, err := os.CreateTemp("", "eucc_download_err_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
