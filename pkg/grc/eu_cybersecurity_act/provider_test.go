...
func TestProviderName(t *testing.T) {
	p := &Provider{}
	if got := p.Name(); got != "eu_cybersecurity_act" {
		t.Errorf("Name() = %q, want %q", got, "eu_cybersecurity_act")
	}
}
...
func TestBuildControl(t *testing.T) {
	p := &Provider{}
	ctrl := p.buildControl(csaRequirement{
		ID: "CSA-99", Title: "Test", Family: "Test Family",
		Description: "Desc", Level: "high", Category: "TestCategory",
	})
	if ctrl.ControlID != "CSA-99" {
		t.Errorf("expected ControlID CSA-99, got %s", ctrl.ControlID)
	}
	...
	if ctrl.References[0].Section != "TestCategory" {
		t.Errorf("expected Section TestCategory, got %s", ctrl.References[0].Section)
	}
}
...
func TestParseSuccess(t *testing.T) {
	catalog := csaCatalog{
		Requirements: []csaRequirement{
			{ID: "CSA-01", Title: "Test", Family: "Family", Description: "Desc", Level: "critical", Category: "Gov"},
		},
	}
...
func TestDownloadSuccess(t *testing.T) {
	catalog := csaCatalog{Requirements: []csaRequirement{{ID: "01", Title: "T"}}}
	data, _ := json.Marshal(catalog)
	...
	f, err := os.CreateTemp("", "csa_download_*.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDownloadNon200(t *testing.T) {
	...
	p := &Provider{logger: testLogger()}
	f, err := os.CreateTemp("", "csa_download_err_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
