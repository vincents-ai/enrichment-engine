...
func TestNew(t *testing.T) {
	...
	if got := p.Name(); got != "toms" {
		t.Errorf("Name() = %q, want %q", got, "toms")
	}
}
...
func TestProviderName(t *testing.T) {
	...
	p := New(mock, nil)
	if p.Name() != "toms" {
		t.Errorf("expected name 'toms', got %q", p.Name())
	}
}
...
func TestEmbeddedControlsFamilies(t *testing.T) {
	...
	if families["Technical Measures"] == 0 {
		t.Error("expected controls in 'Technical Measures' family")
	}
	if families["Organizational Measures"] == 0 {
		t.Error("expected controls in 'Organizational Measures' family")
	}
}

func TestEmbeddedControlsGDPRReferences(t *testing.T) {
	controls := embeddedControls()
	gdprRefCount := 0
	for _, ctrl := range controls {
		for _, ref := range ctrl.References {
			if strings.Contains(ref.Source, "GDPR") {
				gdprRefCount++
			}
		}
	}
	if gdprRefCount == 0 {
		t.Error("expected some controls to reference GDPR articles")
	}
}

func TestEmbeddedControlsLevels(t *testing.T) {
	controls := embeddedControls()
	...
	if levels["critical"] == 0 {
		t.Error("expected some controls with 'critical' level")
	}
	if levels["high"] == 0 {
		t.Error("expected some controls with 'high' level")
	}
}
