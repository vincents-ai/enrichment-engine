...
func TestProviderName(t *testing.T) {
	...
	if p.Name() != "ropa" {
		t.Errorf("expected name 'ropa', got %q", p.Name())
	}
}

func TestFrameworkID(t *testing.T) {
	if FrameworkID != "ROPA_GDPR_ART30" {
		t.Errorf("expected FrameworkID 'ROPA_GDPR_ART30', got %q", FrameworkID)
	}
}
...
func TestEmbeddedControlsFamilies(t *testing.T) {
	...
	expectedFamilies := []string{"Controller Records", "Processor Records", "Processing Templates"}
