...
func TestProviderName(t *testing.T) {
	...
	if p.Name() != "k8s_terraform" {
		t.Errorf("expected name 'k8s_terraform', got %q", p.Name())
	}
}

func TestFrameworkID(t *testing.T) {
	if FrameworkID != "K8S_TF_V1" {
		t.Errorf("expected FrameworkID 'K8S_TF_V1', got %q", FrameworkID)
	}
}
...
func TestEmbeddedControlsFamilies(t *testing.T) {
	...
	expectedFamilies := []string{"Kubernetes", "Terraform/IaC"}
