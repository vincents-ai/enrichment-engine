...
func TestProviderName(t *testing.T) {
	...
	if p.Name() != "iam" {
		t.Errorf("expected name 'iam', got %q", p.Name())
	}
}

func TestFrameworkID(t *testing.T) {
	if FrameworkID != "IAM_V1" {
		t.Errorf("expected FrameworkID 'IAM_V1', got %q", FrameworkID)
	}
}
...
func TestEmbeddedControlsFamilies(t *testing.T) {
	...
	expectedFamilies := []string{"Active Directory", "Entra ID", "Okta", "General IAM"}
