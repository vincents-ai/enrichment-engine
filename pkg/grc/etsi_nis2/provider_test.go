...
func TestProviderName(t *testing.T) {
	p := &Provider{}
	if got := p.Name(); got != "etsi_nis2" {
		t.Errorf("Name() = %q, want %q", got, "etsi_nis2")
	}
}
...
func TestParse(t *testing.T) {
	catalog := nis2Catalog{
		Controls: []nis2Control{
			{ID: "NIS2-01", Title: "Risk Analysis", Family: "Risk", Description: "Desc", Level: "critical", NIS2Article: "21(2)(a)"},
			{ID: "NIS2-02", Title: "Incident Handling", Family: "Incident", Description: "Desc 2"},
		},
	}
	...
	if controls[0].ControlID != "NIS2-01" {
		t.Errorf("expected ControlID NIS2-01, got %s", controls[0].ControlID)
	}
	...
	if !strings.Contains(controls[0].ImplementationGuidance, "21(2)(a)") {
		t.Errorf("expected ImplementationGuidance to contain article, got %s", controls[0].ImplementationGuidance)
	}
}
...
func TestBuildControl(t *testing.T) {
	p := &Provider{}
	ctrl := p.buildControl(nis2Control{
		ID: "NIS2-99", Title: "Test Control", Family: "Test",
		Description: "Test Desc", Level: "high", NIS2Article: "21(2)(g)",
	})
	if ctrl.ControlID != "NIS2-99" {
		t.Errorf("expected ControlID NIS2-99, got %s", ctrl.ControlID)
	}
	...
	if ctrl.References[0].Section != "21(2)(g)" {
		t.Errorf("expected section '21(2)(g)', got %s", ctrl.References[0].Section)
	}
}

func TestDownloadSuccess(t *testing.T) {
	body := `{"controls":[{"id":"NIS2-01","title":"Test","description":"desc"}]}`
	...
	f, err := os.CreateTemp("", "etsi_nis2_download_*.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDownloadNon200(t *testing.T) {
	...
	p := &Provider{logger: testLogger()}
	f, err := os.CreateTemp("", "etsi_nis2_download_err_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
...
func TestRunDownloadSuccess(t *testing.T) {
	catalog := nis2Catalog{
		Controls: []nis2Control{
			{ID: "NIS2-01", Title: "Risk Analysis", Description: "Desc 1", Level: "critical"},
			{ID: "NIS2-02", Title: "Incident Handling", Description: "Desc 2"},
		},
	}
