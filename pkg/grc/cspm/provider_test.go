...
func TestFrameworkID(t *testing.T) {
	if FrameworkID != "CSPM_V1" {
		t.Errorf("expected FrameworkID 'CSPM_V1', got %q", FrameworkID)
	}
}
...
func TestEmbeddedControlsFamilies(t *testing.T) {
	...
	expectedFamilies := []string{"Identity & Access", "Network Security", "Data Protection", "Logging & Monitoring", "Compute Security", "Compliance"}
