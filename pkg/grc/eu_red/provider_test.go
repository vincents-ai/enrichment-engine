...
func TestProviderName(t *testing.T) {
	p := &Provider{}
	if got := p.Name(); got != "eu_red" {
		t.Errorf("Name() = %q, want %q", got, "eu_red")
	}
}
...
func TestParse(t *testing.T) {
	catalog := redCatalog{
		Requirements: []redRequirement{
			{ID: "RED-01", Title: "Network Protection", Family: "Network", Description: "Desc", Level: "critical", Article: "3(3)(d)"},
			{ID: "RED-02", Title: "Privacy Protection", Family: "Privacy", Description: "Desc 2"},
		},
	}
	...
	if controls[0].ControlID != "RED-01" {
		t.Errorf("expected ControlID RED-01, got %s", controls[0].ControlID)
	}
	if len(controls[0].References) != 1 {
		t.Errorf("expected 1 reference, got %d", len(controls[0].References))
	}
	if controls[0].References[0].Section != "3(3)(d)" {
		t.Errorf("expected section 3(3)(d), got %s", controls[0].References[0].Section)
	}
}
...
func TestBuildControl(t *testing.T) {
	...
	ctrl := p.buildControl(req)
	if ctrl.ControlID != "RED-99" {
		t.Errorf("expected ControlID RED-99, got %s", ctrl.ControlID)
	}
	...
	if ctrl.References[0].Section != "3(3)(e)" {
		t.Errorf("expected section 3(3)(e), got %s", ctrl.References[0].Section)
	}
}

func TestDownloadSuccess(t *testing.T) {
	body := `{"requirements":[{"id":"RED-01","title":"Test","description":"desc"}]}`
	...
	f, err := os.CreateTemp("", "eu_red_download_*.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDownloadNon200(t *testing.T) {
	...
	p := &Provider{logger: testLogger()}
	f, err := os.CreateTemp("", "eu_red_download_err_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
...
func TestRunDownloadSuccess(t *testing.T) {
	catalog := redCatalog{
		Requirements: []redRequirement{
			{ID: "RED-01", Title: "Req 1", Description: "Desc 1", Level: "critical"},
			{ID: "RED-02", Title: "Req 2", Description: "Desc 2"},
		},
	}
