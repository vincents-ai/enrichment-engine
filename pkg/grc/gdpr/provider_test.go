...
func TestNew(t *testing.T) {
	...
	if got := p.Name(); got != "gdpr" {
		t.Errorf("Name() = %q, want %q", got, "gdpr")
	}
}
...
func TestToControl(t *testing.T) {
	...
	if ctrl.ControlID != "A5-1" {
		t.Errorf("expected ControlID A5-1, got %s", ctrl.ControlID)
	}
	if ctrl.Framework != FrameworkID {
		t.Errorf("expected Framework %s, got %s", FrameworkID, ctrl.Framework)
	}
	if ctrl.Family != "Principles" {
		t.Errorf("expected Family 'Principles', got %s", ctrl.Family)
	}
	if ctrl.Level != "high" {
	...
	if ctrl.References[0].Source != "GDPR Regulation (EU) 2016/679" {
		t.Errorf("unexpected reference source: %s", ctrl.References[0].Source)
	}
...
func TestToControlDefaultLevel(t *testing.T) {
	p := &Provider{}
	ctrl := p.toControl(gdprControl{
		ID:    "A5-1",
		Title: "Test",
...
func TestParseWithGroups(t *testing.T) {
	catalog := gdprCatalog{
		Groups: []gdprGroup{
			{
				ID:      "art5",
				Title:   "Principles (Article 5)",
				Article: "Article 5",
				Controls: []gdprControl{
					{ID: "A5-1", Title: "Lawfulness", Description: "Processed lawfully"},
					{ID: "A5-2", Title: "Purpose limitation", Description: "Limited purpose"},
				},
			},
		},
	}
	data, _ := json.Marshal(catalog)
	f, err := os.CreateTemp("", "gdpr_test_groups_*.json")
	if err != nil {
		t.Fatal(err)
	}
	...
	if controls[0].ControlID != "A5-1" {
		t.Errorf("expected ControlID A5-1, got %s", controls[0].ControlID)
	}
	if controls[0].Family != "Principles (Article 5)" {
...
func TestParseWithFlatControls(t *testing.T) {
	catalog := gdprCatalog{
		Controls: []gdprControl{
			{ID: "A5-1", Title: "Lawfulness", Article: "Article 5"},
			{ID: "A5-2", Title: "Purpose limitation", Article: "Article 6"},
		},
	}
	data, _ := json.Marshal(catalog)
	f, err := os.CreateTemp("", "gdpr_test_flat_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
	...
	if controls[0].Family != "Article 5" {
		t.Errorf("expected Family from article for flat controls, got %s", controls[0].Family)
	}
}

func TestParseInvalidJSON(t *testing.T) {
	f, err := os.CreateTemp("", "gdpr_test_bad_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
...
func TestDownloadSuccess(t *testing.T) {
	...
	f, err := os.CreateTemp("", "gdpr_download_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
...
func TestDownloadNon200(t *testing.T) {
	...
	p := &Provider{logger: testLogger()}
	f, err := os.CreateTemp("", "gdpr_download_err_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
