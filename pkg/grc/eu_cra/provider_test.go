...
func TestProviderName(t *testing.T) {
	p := &Provider{}
	if got := p.Name(); got != "eu_cra" {
		t.Errorf("Name() = %q, want %q", got, "eu_cra")
	}
}
...
func TestParse(t *testing.T) {
	catalog := craCatalog{
		Requirements: []craRequirement{
			{ID: "R-1", Title: "Req 1", Description: "Desc 1", Level: "critical"},
		},
		AnnexI: []craRequirement{
			{ID: "AI-1", Title: "Annex I Control", Description: "Desc AI"},
		},
		AnnexII: []craRequirement{
			{ID: "AII-1", Title: "Annex II Control", Description: "Desc AII"},
		},
	}
	...
	if controls[0].ControlID != "R-1" {
		t.Errorf("expected ControlID R-1, got %s", controls[0].ControlID)
	}
	if controls[1].Family != "Annex I - Essential Requirements" {
		t.Errorf("expected Family 'Annex I - Essential Requirements', got %s", controls[1].Family)
	}
	if controls[2].Family != "Annex II - Critical Categories" {
		t.Errorf("expected Family 'Annex II - Critical Categories', got %s", controls[2].Family)
	}
}
...
func TestBuildControl(t *testing.T) {
	...
	tests := []struct {
		...
		{"with level", craRequirement{ID: "A1-1", Level: "standard"}, "standard"},
		{"annex II default", craRequirement{ID: "B1-1", Annex: "II"}, "critical"},
		{"annex I default", craRequirement{ID: "A1-1", Annex: "I"}, "high"},
		{"empty annex default", craRequirement{ID: "X-1"}, "high"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctrl := p.buildControl(tt.req)
			if ctrl.ControlID != tt.req.ID {
				t.Errorf("expected ControlID %s, got %s", tt.req.ID, ctrl.ControlID)
			}
			if ctrl.Framework != FrameworkID {
				t.Errorf("expected Framework %s, got %s", FrameworkID, ctrl.Framework)
			}
			if ctrl.Level != tt.wantLevel {
				t.Errorf("expected Level %s, got %s", tt.wantLevel, ctrl.Level)
			}
...
func TestDownloadSuccess(t *testing.T) {
	body := `{"requirements":[{"id":"R-1","title":"Test","description":"desc"}]}`
	...
	f, err := os.CreateTemp("", "eu_cra_download_*.json")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDownloadNon200(t *testing.T) {
	...
	p := &Provider{logger: testLogger()}
	f, err := os.CreateTemp("", "eu_cra_download_err_*.json")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(f.Name())
...
func TestRunDownloadSuccess(t *testing.T) {
	catalog := craCatalog{
		Requirements: []craRequirement{
			{ID: "R-1", Title: "Req 1", Description: "Desc 1", Level: "high"},
			{ID: "R-2", Title: "Req 2", Description: "Desc 2"},
		},
	}
