...
func TestControlsCount(t *testing.T) {
	...
	if len(controls) < 8 {
		t.Errorf("expected at least 8 controls, got %d", len(controls))
	}
}

func TestAllControlsFrameworkID(t *testing.T) {
	controls := staticControls()
...
func TestNoEmptyTitlesOrIDs(t *testing.T) {
	controls := staticControls()
	for _, ctrl := range controls {
		if ctrl.ControlID == "" {
...
func TestProviderName(t *testing.T) {
	p := New(nil, nil)
	if got := p.Name(); got != "enisa_supply_chain" {
		t.Errorf("Name() = %q, want %q", got, "enisa_supply_chain")
	}
}
...
func TestProviderRun(t *testing.T) {
	...
	if count < 8 {
		t.Errorf("expected at least 8 controls written, got %d", count)
	}
}
